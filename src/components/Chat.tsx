...
export default function Chat({ sessionId: propSessionId }: ChatProps) {
  ...
  const [pendingPermission, setPendingPermission] = useState<PermissionInfo | null>(null)
  ...
  const [serverState, setServerState] = useState<any>({ messages: [], permissions: { byId: {}, queue: [] }, lastUpdate: 0 })
  ...
  const { modelList, pricingByModel, contextLimitByModel } = React.useMemo(() => {
    const result = {
      modelList: [] as Array<{ id: string; name: string; provider: string }>,
      pricingByModel: {} as Record<string, { input?: number; output?: number; reasoning?: number; cache_read?: number; cache_write?: number }>,
    ...
    for (const provider of providers.providers) {
      for (const [id, model] of Object.entries<any>(provider.models || {})) {
        ...
        result.modelList.push({ id, name: model?.name, provider: provider.id })
        if (model?.cost && typeof model.cost === 'object') {
          result.pricingByModel[id] = {
            ...
            reasoning: typeof model.cost.reasoning === 'number' ? model.cost.reasoning : undefined,
            cache_read: typeof model.cost.cache_read === 'number' ? model.cost.cache_read : undefined,
            cache_write: typeof model.cost.cache_write === 'number' ? model.cost.cache_write : undefined,
          }
        }
        if (model?.limit && typeof model.limit.context === 'number') {
          result.contextLimitByModel[id] = model.limit.context
          result.contextLimitByModel[`${provider.id}/${id}`] = model.limit.context
        }
      }
    }
    return result
  ...
  useEffect(() => {
    ...
    const exists = currentId ? modelList.some((m: any) => m.id === currentId) : false
    if (!exists) {
      const saved = localStorage.getItem('opencodePreferredModel')
      const defaultModel = 'claude-opus-4-1-20250805'
      const modelToUse = (modelList as any[]).find(m => m.id === saved) || (modelList as any[]).find(m => m.id === defaultModel) || (modelList as any[])[0]
      if (modelToUse) {
        const modelValue = JSON.stringify({ id: (modelToUse as any).id, provider: (modelToUse as any).provider })
  ...
  useEffect(() => {
    ...
    const lastAssistant = [...msgs].reverse().find((m: any) => m?.info?.role === 'assistant' && m?.info?.modelID)
    ...
    const provider = providerId || ((modelList as any[]).find((m: any) => m.id === modelId) as any)?.provider
  ...
  useEffect(() => {
    if (!sessionId) {
      setServerState({ messages: [], permissions: { byId: {}, queue: [] }, lastUpdate: 0 })
        ...
        try {
          ...
          if ((state?.lastUpdate || 0) > (serverState?.lastUpdate || 0)) {
            ...
            if (lastAssistant?.info?.tokens) {
              const total = (lastAssistant.info.tokens.input || 0) + (lastAssistant.info.tokens.output || 0) + (lastAssistant.info.tokens.reasoning || 0)
              const modelKey = lastAssistant.info.modelID
              const limit = (contextLimitByModel as any)[modelKey] ?? (contextLimitByModel as any)[(modelKey || '').split('/').pop()!] ?? 200000
            ...
            if ((lastCost === undefined || lastCost === 0) && lastAssistant) {
              ...
              for (const p of (lastAssistant.parts || [])) {
                if (typeof (p as any)?.cost === 'number') partsCost += (p as any).cost as number
              }
              if (partsCost > 0) lastCost = partsCost
            }
            if ((lastCost === undefined || lastCost === 0) && lastAssistant?.info?.tokens && lastAssistant?.info?.modelID) {
              ...
              if (price) {
                ...
                if (price.reasoning) computed += (tReason / denom) * price.reasoning
                if (computed > 0) lastCost = computed
              }
            }
            if (typeof lastCost === 'number') setCost(lastCost)
            const lastMsg = (msgs || []).slice(-1)[0]
            const isLastMessageIncomplete = lastMsg?.info?.role === 'assistant' && !lastMsg?.info?.time?.completed
            const hasRunningTools = isLastMessageIncomplete && (lastMsg?.parts || []).some((p: any) =>
  ...
  const sendMessage = useMutation({
    ...
    onSuccess: (data) => {
      ...
      setInput('')
      queryClient.invalidateQueries({ queryKey: ['messages', data.sessionId || sessionId] })
    },
    onError: () => {
      setIsThinking(false)
      setIsResponding(false)
  ...
  const handlePermissionResponse = async (response: 'once' | 'always' | 'reject') => {
    ...
    try {
      ...
      const res = await fetch(`/session/${pendingPermission.sessionID}/permissions/${permissionId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <>
      ...
      {(() => {
        ...
        const isInline = !!pendingPermission.callID && (serverState.messages || []).some((m: any) =>
          (m.parts || []).some((p: any) => p.type === 'tool' && p.callID === pendingPermission.callID)
        )
      ...
      <div className="px-4 py-2 bg-gray-900 text-xs text-gray-400 flex justify-between items-center flex-shrink-0 sm:rounded-b-2xl">
        ...
        {Math.max(0, (serverState.permissions?.queue?.length || 0) - (pendingPermission ? 1 : 0)) > 0 && (
          <span className="ml-2 text-[10px] bg-yellow-500 text-black px-1.5 py-0.5 rounded-full" title="Queued approval requests">
            +{Math.max(0, (serverState.permissions?.queue?.length || 0) - (pendingPermission ? 1 : 0))}
          </span>
        )}
        ...
        <div className="flex gap-2">
          ...
          >
            ...
            {Object.entries(
              (modelList as Array<{id: string; name: string; provider: string}>).reduce((acc: Record<string, Array<{id: string; name: string; provider: string}>>, model) => {
              ...
              <optgroup key={provider} label={provider}>
                {(providerModels as Array<{id: string; name: string; provider: string}>).map((model) => (
                  <option key={model.id} value={JSON.stringify({id: model.id, provider: model.provider})}>
                    {model.name}
                  </option>
                ))}
              </optgroup>
          ...
          <select 
            ...
            className="bg-gray-800 text-gray-400 border border-gray-700 rounded px-2 py-1"
          >
            {agents?.filter((a: any) => a.mode === 'primary').map((agent: any) => (
